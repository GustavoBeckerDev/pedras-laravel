...
@section('content')
            ...
            <div class="flex justify-center md:justify-end animate-fade-in-right">
                @foreach ($pedras as $pedra)
                    <img src="{{ asset('storage/' . $pedra->imagem) }}"
                            alt="{{ $pedra->nome }}"
                            class="stone hidden w-96 md:w-[32rem] drop-shadow-[0_0_25px_rgba(168,85,247,0.8)]
                            transform -translate-x-6 md:-translate-x-12 transition-opacity duration-500 opacity-0">
                @endforeach
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            if (stones.length > 0) {
                stones[0].classList.remove("hidden");
                stones[0].classList.replace("opacity-0", "opacity-100");

                setInterval(() => {
                    stones[index].classList.add("hidden");
                    stones[index].classList.replace("opacity-100", "opacity-0");

                    index = (index + 1) % stones.length;

                    stones[index].classList.remove("hidden");
                    stones[index].classList.replace("opacity-0", "opacity-100");
                }, 4000);
            }
        });
    </script>
